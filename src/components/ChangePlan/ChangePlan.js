...
const ChangePlan = ({ location, dependencies: { planService } }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const pathList = await planService.getPaths(currentPlan);
                ...
                state.pathList.map((path, index) => (
                  <Card highlighted={path.type === 'plus'} key={index}>
                    ...
                    >
                      ...
                      <p>{_('change_plan.until_x_subscribers', { subscribers: 1500 })}</p>
                    </div>
                    ...
                    {path.type === 'free' ? (
                      <span class="dp-current-plan"> Plan Actual </span>
                    ) : path.type === 'agencies' ? (
                      <>
                      <img className='dp-price'
                        style={{ width: '80px' }}
